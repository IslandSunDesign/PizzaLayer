...
/**
 * PizzaLayer — Nutrition & Ingredients meta box.
 *
 ...
 * Storage
 * -------
 * All values are written to the plugin's canonical `_pizzalayer_{key}` post-meta
 ...
 *
 *  - _pizzalayer_ingredients      (string, newline-separated — one ingredient per line)
 *  - _pizzalayer_serving_size     (string, e.g. "1 slice")
 *  - _pizzalayer_calories         (int)
 *  - _pizzalayer_spice_level      (mild|medium|hot|extra_hot)
 *  - _pizzalayer_thickness        (string, crusts only)
 *  - _pizzalayer_is_vegetarian / _is_vegan / _is_gluten_free / _is_dairy_free ('1' | '')
 *
...
class NutritionMetaBox {
	...
	public function add_meta_boxes(): void {
		foreach ( array_keys( self::TYPE_FIELDS ) as $slug ) {
			add_meta_box(
				'pizzalayer_nutrition',
				'<span class="dashicons dashicons-carrot" style="color:#ff6b35;font-size:14px;width:14px;height:14px;vertical-align:middle;margin-right:5px;"></span> ' . esc_html__( 'Nutrition & Ingredients', 'pizzalayer' ),
				[ $this, 'render' ],
				'pizzalayer_' . $slug,
				'normal',
				'default',
	...
	public function render( \WP_Post $post, array $box ): void {
		...
		wp_nonce_field( 'pizzalayer_nutrition_save', 'pizzalayer_nutrition_nonce' );

		$pid = (int) $post->ID;
		$get = function ( string $key ) use ( $pid ) {
			return (string) get_post_meta( $pid, '_pizzalayer_' . $key, true );
		};
		?>
		<div class="pzl-nutri" style="max-width:760px;">
			<table class="form-table" role="presentation">
				...
				<tr>
					<th scope="row">
						<label for="pzl-nutri-ingredients"><?php esc_html_e( 'Ingredients', 'pizzalayer' ); ?></label>
					</th>
					<td>
						<textarea id="pzl-nutri-ingredients" name="pzl_nutrition[ingredients]" rows="5" class="large-text code" placeholder="<?php esc_attr_e( "One ingredient per line, e.g.\nMozzarella\nTomato\nBasil", 'pizzalayer' ); ?>"><?php echo esc_textarea( $get( 'ingredients' ) ); ?></textarea>
						<p class="description"><?php esc_html_e( 'List each ingredient on its own line. Used for allergen / nutrition display and available to templates.', 'pizzalayer' ); ?></p>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<th scope="row"><label for="pzl-nutri-serving"><?php esc_html_e( 'Serving Size', 'pizzalayer' ); ?></label></th>
					<td>
						<input type="text" id="pzl-nutri-serving" name="pzl_nutrition[serving_size]" class="regular-text" value="<?php echo esc_attr( $get( 'serving_size' ) ); ?>" placeholder="<?php esc_attr_e( 'e.g. 1 slice', 'pizzalayer' ); ?>">
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<th scope="row"><label for="pzl-nutri-calories"><?php esc_html_e( 'Calories', 'pizzalayer' ); ?></label></th>
					<td>
						...
						<span class="description"><?php esc_html_e( 'kcal per serving', 'pizzalayer' ); ?></span>
					</td>
				</tr>
				<?php endif; ?>
				...
				<?php if ( in_array( 'spice_level', $fields, true ) ) :
					...
					$opts  = [ '' => __( '— None —', 'pizzalayer' ), 'mild' => __( 'Mild', 'pizzalayer' ), 'medium' => __( 'Medium', 'pizzalayer' ), 'hot' => __( 'Hot', 'pizzalayer' ), 'extra_hot' => __( 'Extra Hot', 'pizzalayer' ) ];
				?>
				<tr>
					<th scope="row"><label for="pzl-nutri-spice"><?php esc_html_e( 'Spice Level', 'pizzalayer' ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="pzl-nutri-thickness"><?php esc_html_e( 'Thickness', 'pizzalayer' ); ?></label></th>
					<td>
						<input type="text" id="pzl-nutri-thickness" name="pzl_nutrition[thickness]" class="regular-text" value="<?php echo esc_attr( $get( 'thickness' ) ); ?>" placeholder="<?php esc_attr_e( 'e.g. thin, regular, deep-dish', 'pizzalayer' ); ?>">
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Dietary', 'pizzalayer' ); ?></th>
					<td>
						<fieldset>
	...
	public function save( int $post_id, \WP_Post $post ): void {
		...
		$slug = str_replace( 'pizzalayer_', '', (string) $post->post_type );
		if ( ! isset( self::TYPE_FIELDS[ $slug ] ) ) { return; }

		if ( ! isset( $_POST['pizzalayer_nutrition_nonce'] )
		     || ! wp_verify_nonce( sanitize_key( $_POST['pizzalayer_nutrition_nonce'] ), 'pizzalayer_nutrition_save' ) ) {
			return;
		}
		...
		if ( in_array( 'ingredients', $fields, true ) ) {
			...
			update_post_meta( $post_id, '_pizzalayer_ingredients', implode( "\n", $clean ) );
		}

		if ( in_array( 'serving_size', $fields, true ) ) {
			update_post_meta( $post_id, '_pizzalayer_serving_size', sanitize_text_field( $in['serving_size'] ?? '' ) );
		}

		if ( in_array( 'calories', $fields, true ) ) {
			$cal = isset( $in['calories'] ) && $in['calories'] !== '' ? max( 0, (int) $in['calories'] ) : '';
			update_post_meta( $post_id, '_pizzalayer_calories', $cal === '' ? '' : (string) $cal );
		}
		...
		if ( in_array( 'spice_level', $fields, true ) ) {
			...
			update_post_meta( $post_id, '_pizzalayer_spice_level', $spice );
		}

		if ( in_array( 'thickness', $fields, true ) ) {
			update_post_meta( $post_id, '_pizzalayer_thickness', sanitize_text_field( $in['thickness'] ?? '' ) );
		}
		...
		if ( in_array( 'dietary', $fields, true ) ) {
			foreach ( array_keys( self::DIETARY ) as $key ) {
				...
				update_post_meta( $post_id, '_pizzalayer_' . $key, $on ? '1' : '' );
			}
		}
	}
	 ...
	 *   PizzaLayer\Admin\NutritionMetaBox::get_ingredients( $post_id );
	 *
	 * @return string[]
	 */
	public static function get_ingredients( int $post_id ): array {
		$raw = (string) get_post_meta( $post_id, '_pizzalayer_ingredients', true );
		if ( $raw === '' ) { return []; }
		$lines = preg_split( '/\r\n|\r|\n/', $raw );
		$out   = [];
		foreach ( (array) $lines as $line ) {
