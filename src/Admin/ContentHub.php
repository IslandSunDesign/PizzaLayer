...
class ContentHub {
	...
	public function render(): void {
		...
		<?php $this->render_scripts( $active_slug, $nonce ); ?>
		<?php
	}
	...
	private function render_scripts( string $active_slug, string $nonce ): void {
		$cpt_data = [];
		...
		(function(){
			...
			var CPT_DATA = <?php echo wp_json_encode( $cpt_data ); ?>;
			var current  = <?php echo wp_json_encode( $active_slug ); ?>;
			...
			// Cache for loaded panel HTML (avoid re-fetching)
			var panelCache   = {};
			panelCache[current] = $panel.innerHTML;

			function showLoading(){ $loading.style.display = 'flex'; $panel.classList.add('plch-fading'); }
			function hideLoading(){ $loading.style.display = 'none'; $panel.classList.remove('plch-fading'); }
			...
			function setActiveRailItem(slug){
				...
				// Update header
				$headerIcon.className = 'dashicons ' + meta.icon + ' plch-header-icon';
				$headerIcon.style.color = meta.color;
				$headerLabel.textContent = meta.label;
				$headerDesc.textContent  = meta.desc;
				$addBtn.href = meta.addUrl;
				$addSingular.textContent = meta.singular;
				$header.style.borderColor = meta.color + '60';
			}
			...
			function loadPanel(slug){
				...
				if (panelCache[slug]) {
					...
					$panel.innerHTML = panelCache[slug];
				...
				// Mark loading state
				document.querySelectorAll('.plch-rail__item').forEach(function(el){
					el.classList.toggle('plch-rail__item--loading', el.getAttribute('data-slug') !== current && el.getAttribute('data-slug') !== slug);
				});
				showLoading();
			...
			function reinitTableLinks(slug){
				// Rewrite column-sort links to stay within the hub page
			...
			// Intercept rail clicks
			if ($rail) {
				$rail.addEventListener('click', function(e){
					var item = e.target.closest('.plch-rail__item');
					if (!item) return;

					// Don't intercept the + Add New sub-link
			...
			// Init table links for the initial panel
