...
class PizzaRestApi {
	 ...
	 *
	 *   pizzalayer_rest_rate_limit   — max requests per window (default 120)
	 *   pizzalayer_rest_rate_window  — window length in seconds (default 60)
	 *
	...
	public function check_public_access( \WP_REST_Request $request ) {
		$max    = (int) apply_filters( 'pizzalayer_rest_rate_limit', 120 );
		$window = (int) apply_filters( 'pizzalayer_rest_rate_window', 60 );
		...
		if ( (int) $entry['count'] >= $max ) {
			return new \WP_Error(
				'pizzalayer_rate_limited',
				__( 'Too many requests. Please slow down and try again shortly.', 'pizzalayer' ),
				[ 'status' => 429 ]
			);
		}
	...
	public function render_pizza( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$ttl = (int) get_option( 'pizzalayer_setting_adv_rest_cache_ttl', 0 );
		if ( $ttl > 0 ) {
			$cache_key = 'pizzalayer_render_' . md5( (string) wp_json_encode( $layers ) );
