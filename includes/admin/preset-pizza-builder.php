...
function pizzalayer_render_preset_metabox( $post ) {
    ...
    $fields = array(
        'crust'    => ['label'=>'Crust',    'cpt'=>'pizzalayer_crusts',    'meta_key'=>'preset_chosen_crust',    'single'=>true,  'img'=>'crust_layer_image'],
        'sauce'    => ['label'=>'Sauce',    'cpt'=>'pizzalayer_sauces',    'meta_key'=>'preset_chosen_sauce',    'single'=>true,  'img'=>'sauce_layer_image'],
        'cheese'   => ['label'=>'Cheese',   'cpt'=>'pizzalayer_cheeses',   'meta_key'=>'preset_chosen_cheese',   'single'=>true,  'img'=>'cheese_layer_image'],
        'toppings' => ['label'=>'Toppings','cpt'=>'pizzalayer_toppings',   'meta_key'=>'preset_chosen_toppings', 'single'=>false, 'img'=>'topping_list_image'],
        'drizzle'  => ['label'=>'Drizzle',  'cpt'=>'pizzalayer_drizzles',  'meta_key'=>'preset_chosen_drizzle',  'single'=>true,  'img'=>'drizzle_image'],
        'cut'      => ['label'=>'Slices',   'cpt'=>'pizzalayer_cuts',      'meta_key'=>'preset_chosen_cut',      'single'=>true,  'img'=>'cut_layer_image'],
    );

    foreach ( $fields as $key => $args ) {
        $saved = get_field( $args['meta_key'], $post->ID );
        ...
        foreach ( $items as $item ) {
            ...
            $img_id      = get_field( $args['img'], $item->ID );
           // $img_url     = $img_id ? wp_get_attachment_image_url( $img_id, 'thumbnail' ) : '';
           $img_url      = get_field( $args['img'], $item->ID );
...
/* +===  Register the Selected Layers Meta Box  ===+ */
add_action( 'add_meta_boxes', 'pizzalayer_add_selected_layers_metabox' );
function pizzalayer_add_selected_layers_metabox() {
    add_meta_box(
        'pizzalayer_selected_layers',
        __( 'Selected Layers', 'pizzalayer' ),
        'pizzalayer_render_selected_layers_metabox',
        'pizzalayer_pizzas',
        'normal', // full-width like ACF
        'high'
    );
}

/* +===  Render the Meta Box Fields  ===+ */
function pizzalayer_render_selected_layers_metabox( $post ) {
    $fields = [
        'preset_chosen_crust'    => 'Crust',
        'preset_chosen_sauce'    => 'Sauce',
        'preset_chosen_cheese'   => 'Cheese',
        'preset_chosen_drizzle'  => 'Drizzle',
        'preset_chosen_cut'      => 'Cut',
        'preset_chosen_toppings' => 'Toppings',
    ];
    ...
    wp_nonce_field( 'pizzalayer_save_selected_layers', 'pizzalayer_selected_layers_nonce' );
}

/* +===  Save the Meta Box Field Values  ===+ */
add_action( 'save_post_pizzalayer_pizzas', 'pizzalayer_save_selected_layers_metabox' );
function pizzalayer_save_selected_layers_metabox( $post_id ) {
    if ( ! isset( $_POST['pizzalayer_selected_layers_nonce'] ) || 
         ! wp_verify_nonce( $_POST['pizzalayer_selected_layers_nonce'], 'pizzalayer_save_selected_layers' ) ) {
        return;
    }
