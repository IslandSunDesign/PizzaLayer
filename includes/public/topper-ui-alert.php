...
function pizzalayer_alert($pizzalayer_alert_content,$pizzalayer_alert_style){
    ...
    do_action( 'func_pizzalayer_alert_before_return' );
...
function pizzalayer_demo_notice(){
    if( get_option('pizzalayer_setting_settings_demonotice') ){
        return '<div class="pizzalayer-ui-menu-col pizzalayer-ui-menu-col-alert col-lg-12 col-md-12">' . get_option('pizzalayer_setting_settings_demonotice') . '</div>'; }
    else {
        return '';
    };
}

do_action( 'pizzalayer_file_topper-ui-alert_after' );