...
// Ensure we have $instance_id and $atts (guard for direct include)
 ...
 */
function np_exclusive_card( $post, string $layer_type, string $np_var, int $zindex = 200 ): string {
	$id        = $post->ID;
 ...
 */
function np_topping_card( $post, string $np_var, int $zindex ): string {
	$id        = $post->ID;
	$title     = get_the_title( $post );
	$slug      = sanitize_title( $title );
